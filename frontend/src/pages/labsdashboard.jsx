...
function Labsdashboard() {
	...
	const gettests = async () => {
		try {
			const response = await axios({
				url: "https://labeasy.onrender.com/api/v1/tests/gettestsforlab",
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}`,
	...
	const handleSubmit = async (e) => {
		...
		try {
			await axios({
				url: "https://labeasy.onrender.com/api/v1/tests/addlabtest",
				method: "POST",
				data: JSON.stringify({
	...
	const handleDelete = async (id) => {
		try {
			const newTests = await axios({
				url: `https://labeasy.onrender.com/api/v1/tests/deletelabtest/${id}`,
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}`,
